...
public class BytecodeProviderImpl implements BytecodeProvider {
	...
	public ReflectionOptimizer getReflectionOptimizer(
		...
		if ( fastClass == null ) {
			fastClass = new ByteBuddy()
					.with( new NamingStrategy.SuffixingRandom( "HibernateInstatiator" ) )
	...
	private static class GetPropertyValues implements ByteCodeAppender {
		...
		@Override
		public Size apply(
				MethodVisitor methodVisitor, Implementation.Context implementationContext, MethodDescription instrumentedMethod) {
			methodVisitor.visitLdcInsn( getters.length );
			methodVisitor.visitTypeInsn( Opcodes.NEWARRAY, Type.getInternalName( Object.class ) );
			int index = 0;
			for ( Method getter : getters ) {
				methodVisitor.visitLdcInsn( Opcodes.DUP );
				...
				if ( getter.getReturnType().isPrimitive() ) {
					PrimitiveUnboxingDelegate.forReferenceType( TypeDescription.Generic.OBJECT )
							.assignUnboxedTo(
									new TypeDescription.Generic.OfNonGenericType.ForLoadedType( getter.getReturnType() ),
		...
		public Size apply(
			...
			for ( Method setter : setters ) {
				...
				if ( setter.getParameterTypes()[0].isPrimitive() ) {
					PrimitiveBoxingDelegate.forPrimitive( new TypeDescription.ForLoadedType( setter.getParameterTypes()[0] ) )
							.assignBoxedTo(
	...
	private static class CloningPropertyCall implements Callable<String[]> {
